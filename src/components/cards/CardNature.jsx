...
export default function CardNature({ data, fullscreen = false }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'rgba(4,15,7,0)' }} />
      ...
      <div className="relative z-10 space-y-4 max-w-lg">
        ...
        <div className="pt-2 space-y-1">
          <p className={`${fullscreen ? 'text-2xl' : 'text-base'} font-script`} style={{ color: '#14532d' }}>
            {signature || 'Avec tout mon amour'}
          </p>
          {senderName && (
            <p className={`${smallText} text-[#15803d]`} style={{ fontFamily: '"Lato", sans-serif' }}>— {senderName}</p>
          )}
        </div>
      </div>
    </div>
